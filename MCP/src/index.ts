...
async function main() {
  ...
  try {
    ...
    process.stderr.write("[lookin-mcp] Auto-connected successfully\n");
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    process.stderr.write(`[lookin-mcp] Auto-connect skipped: ${msg}\n`);
  }

  process.stderr.write("[lookin-mcp] Server started\n");
...
main().catch((err) => {
  process.stderr.write(
    `[lookin-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`
  );
  process.exit(1);
});
