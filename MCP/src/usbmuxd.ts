...
function buildPlistBody(payload: Record<string, string | number>): Buffer {
  const entries = Object.entries(payload).map(([key, value]) => {
    const encodedKey = `<key>${escapeXml(key)}</key>`;
    const encodedValue =
      typeof value === "number"
...
function buildMsg(tag: number, payload: Record<string, string | number>): Buffer {
  const body = buildPlistBody(payload);
